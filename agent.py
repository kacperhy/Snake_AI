...
class BuforDoświadczeń:
    ...
    Atrybuty:
        ...
        urzadzenie (torch.urzadzenie): Urządzenie do obliczeń (CPU/GPU).
        typ_danych (torch.typ_danych): Typ danych dla tensorów.
    """
    def __init__(self, pojemność=EXPERIENCE_BUFFER_SIZE):
        self.bufor = deque(maxlen=pojemność)
        self.urzadzenie = torch.urzadzenie("cuda" if USE_GPU else "cpu")
        self.typ_danych = torch.float16 if USE_FLOAT16 and USE_GPU else torch.float32
    ...
    def próbkuj(self, rozmiar_partii):
        ...
        tensor_stanów = torch.tensor(np.array(stany), typ_danych=self.typ_danych).to(self.urzadzenie)
        tensor_akcji = torch.tensor(akcje, typ_danych=torch.long).to(self.urzadzenie)
        tensor_nagród = torch.tensor(nagrody, typ_danych=self.typ_danych).to(self.urzadzenie)
        tensor_następnych_stanów = torch.tensor(np.array(następne_stany), typ_danych=self.typ_danych).to(self.urzadzenie)
        tensor_zakończonych = torch.tensor(zakończone, typ_danych=torch.bool).to(self.urzadzenie)
    ...
    def _wątek_wstępnego_pobierania(self, rozmiar_partii):
        ...
        partia = self.próbkuj(rozmiar_partii)
...
class DQNAgent:
    ...
    def __init__(self, rozmiar_stanu, rozmiar_akcji, rozmiar_ukryty=256, współczynnik_uczenia=0.0003):
        ...
        self.rozmiar_partii = 256 if USE_GPU else 64  # Większy partia na GPU, mniejszy na CPU
        ...
        self.model = QNetwork(rozmiar_stanu, rozmiar_ukryty, rozmiar_akcji).to(self.urzadzenie)
        if USE_FLOAT16 and USE_GPU:
            self.model = self.model.half()  # Konwersja do float16 dla GPU
        
        # Model sieci docelowej (target network)
        self.model_docelowy = QNetwork(rozmiar_stanu, rozmiar_ukryty, rozmiar_akcji).to(self.urzadzenie)
        if USE_FLOAT16 and USE_GPU:
        ...
        print(f"Używam urządzenia: {self.urzadzenie}")
        if self.urzadzenie.type == 'cuda':
    ...
    def pobierz_akcję(self, state):
        ...
        with torch.no_grad():
            tensor_stanu = torch.tensor(state, typ_danych=self.typ_danych).unsqueeze(0).to(self.urzadzenie)
    ...
    def ucz_się(self):
        ...
        if partia is None:
            ...
            partia = self.pamięć.próbkuj(self.rozmiar_partii)
            if partia is None:
    ...
    def zapisz(self, file_name='models/dqn_model.pth'):
        """
        ...
        stan_modelu = {
            ...
            'device_type': self.urzadzenie.type,
            'use_float16': USE_FLOAT16
        }
        torch.zapisz(stan_modelu, file_name)
    ...
    def wczytaj(self, file_name='models/dqn_model.pth'):
        ...
        """
        try:
            punkt_kontrolny = torch.wczytaj(file_name, map_location=self.urzadzenie)
            ...
            if wczytane_urządzenie != self.urzadzenie.type:
                print(f"UWAGA: Model został wytrenowany na {wczytane_urządzenie}, a obecnie używasz {self.urzadzenie.type}.")
                
            # Obsługa różnych precyzji
            wczytane_float16 = punkt_kontrolny.get('use_float16', False)
            if wczytane_float16 != USE_FLOAT16 and self.urzadzenie.type == 'cuda':
                print(f"UWAGA: Model używał float16={wczytane_float16}, a obecnie float16={USE_FLOAT16}.")
                if USE_FLOAT16 and not wczytane_float16:
                    print("Konwertuję model do half precision (float16)...")
                    self.model = self.model.half()
                elif not USE_FLOAT16 and wczytane_float16:
    ...
    # Zapamiętaj oryginalne urządzenie
        original_device = self.urzadzenie
    ...
    # Obsługa wymuszonego urządzenia
        if force_device:
            ...
            if force_device in ['cpu', 'cuda']:
                new_device = torch.urzadzenie(force_device)
                if new_device != self.urzadzenie:
                    print(f"Przenoszenie modelu z {self.urzadzenie} na {new_device}...")
                
                    # Przenieś modele na nowe urządzenie
                    self.urzadzenie = new_device
                ...
                # Obsługa precyzji dla GPU
                    ...
                    if new_device.type == 'cuda':
                        współczynnik_uczenia = self.optimizer.param_groups[0]['współczynnik_uczenia']
                        self.optimizer = optim.AdamW(self.model.parameters(), współczynnik_uczenia=współczynnik_uczenia, weight_decay=1e-5)
                    else:
                        współczynnik_uczenia = self.optimizer.param_groups[0]['współczynnik_uczenia']
                        self.optimizer = optim.Adam(self.model.parameters(), współczynnik_uczenia=współczynnik_uczenia)
        ...
        print(f"Kontynuuję trening na urządzeniu: {self.urzadzenie}")
        ...
        try:
            ...
            if self.urzadzenie != original_device:
                print(f"Przywracam model na oryginalne urządzenie: {original_device}")
                self.urzadzenie = original_device
